      ...
      stream: result.stream.pipeThrough(
        ...
        >({
          transform(part, controller) {
            ...
            if (part.type === 'finish') {
              // On a completed response an unmatched `<think>`/`<precontext>`
              // is prose, so recover it verbatim; a truncated response drops
              // its partial tag rather than leaking it.
              if (part.finishReason.unified !== 'length') {
                const authoritative = withoutClosedBlocks(rawAccumulated);
                if (
                  authoritative.length > emittedAccumulated.length &&
                  authoritative.startsWith(emittedAccumulated)
                ) {
                  const recovered = authoritative.slice(
                    emittedAccumulated.length,
                  );
                  const id = `${textId ?? 'txt'}-recovered`;
