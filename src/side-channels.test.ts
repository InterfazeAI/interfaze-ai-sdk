...
describe('withoutClosedBlocks', () => {
  it('removes closed blocks without trimming the surrounding text', () => {
    expect(
      withoutClosedBlocks(
        '  a <think>x</think>b <precontext>[]</precontext>c ',
      ),
    ).toBe('  a b c ');
  });

  it('leaves an unterminated tag byte-for-byte intact', () => {
    expect(withoutClosedBlocks('Use the <think> tag to hide reasoning')).toBe(
      'Use the <think> tag to hide reasoning',
    );
  });
});
